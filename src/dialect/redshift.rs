...
/// Redshift-specific transpilation rules.
pub struct RedshiftDialect;

impl Transpiler for RedshiftDialect {
    fn transpile_statement(
        ...
        stmt = match stmt {
            s @ Statement::CreateTable { .. } | s @ Statement::AlterTable(..) => {
                ddl::rewrite_ddl(s, SourceDialect::Redshift, opts)?
            }
            s @ Statement::Insert(_)
            | s @ Statement::Update { .. }
            | s @ Statement::Delete(_)
            | s @ Statement::Merge { .. }
            | s @ Statement::Copy { .. } => dml::rewrite_dml(s, SourceDialect::Redshift, opts)?,
            s if is_show_statement(&s) => show::rewrite_show(s, SourceDialect::Redshift, opts)?,
            s => s,
        };
